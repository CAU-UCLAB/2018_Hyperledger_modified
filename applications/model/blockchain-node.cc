                    ...
                    switch(d["message"].GetInt())
                    {
                        ...
                        case REPLY_TRANS:
                        {
                            ...
                            for(j = 0; j < d["transactions"].Size(); j++)
                            {
                                ...
                                else if(!HasReplyTransaction(nodeId, transId, transExecution) && GetNode()->GetId() != nodeId && m_isMiner != true)
                                {
                                    //if node is Committer or client...
                                ...
                                else if(!HasReplyTransaction(nodeId, transId, transExecution) && m_isMiner == true)
                                {
                                    
                                    //if node is orderer...
                                    //std::cout<<"orderer receives endorsed transaction\n";
                                    if(!HasMessageTransaction(nodeId, transId))
                                    {
                                        
                                        
                                        std::vector<Transaction>::iterator it_tran;
                                        std::map<std::string, std::vector<Transaction>>::iterator it_map;
                                        std::string transHash = std::to_string(nodeId) + std::to_string(transId);
                                        //std::cout<< "nodeid = " << nodeId << " transId = " << transId <<"trans Hash = " << transHash <<"\n";
                                        ...
                                        it_map = m_endorsedTransactions.find(transHash);

                                        if(it_map != m_endorsedTransactions.end())
                                        {
                                            for(it_tran = m_endorsedTransactions[transHash].begin(); it_tran < m_endorsedTransactions[transHash].end() ; it_tran++)
                                            {
                                            ...
                                            if(it_tran == m_endorsedTransactions[transHash].end())
                                            {
                                                m_endorsedTransactions[transHash].push_back(newTrans);
                                                //std::cout<<"Order : the transaction " << transHash << " pushed in buffer\n";
                                            }

                                            if(m_endorsedTransactions[transHash].size() == m_numberofEndorsers)
                                            {
                                                AdvertiseNewTransaction(newTrans, MSG_TRANS, InetSocketAddress::ConvertFrom(from).GetIpv4());
                                                m_endorsedTransactions.erase(transHash);
                                                m_msgTransaction.push_back(newTrans);
                                                m_notValidatedTransaction.push_back(newTrans);
                                                //std::cout<<"Order : the transaction " << transHash << " will be in block\n";
                                            }
                                        ...
                                        if(it_tran == m_waitingEndorsers.end())
                                        {
                                            ...
                                            //std::cout<<"Order : the transaction pushed in buffer\n";
                                        }
                                        ...
                                        if(m_waitingEndorsers.size() == m_numberofEndorsers)
                                        {
                                            ...
                                            m_msgTransaction.push_back(newTrans);
                                            m_notValidatedTransaction.push_back(newTrans);
                                            //std::cout<<"Order : the transaction will be in block\n";
                                        }
                                        */
                                    }
                                    else
                                    {
                                        NS_LOG_INFO("REPLY_TRANS: Blockchain node(Order) " << GetNode()->GetId()
                                                    << "already received same reply message");
                                    }
